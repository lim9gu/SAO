...
#=================================================================
def SAO_info(camera='STX16803'):
	"""
	###### SAO Preprocessing code Ver 0.5
	###### developed by G.Lim (lim9gu@gmail.com)
...
#=================================================================
def SAO_fileset(camera = 'STX16803'):
	"""
	1. Description
	...
	for i in xrange(len(calibrations)):
		...
		if IMAGETYP == 'Bias Frame' :
			bias.append(calibrations[i])
		elif IMAGETYP == 'Dark Frame' :
			dark.append(calibrations[i])
	print('Bias & dark images are classified.')
	#bias = glob.glob('cal*bias.fit')
	if ('zero.fits' not in os.listdir('./')) & (bias == []) & ('bias' in os.listdir('./')):
	...
	dark = glob.glob('*dk*.fit')
	predark = glob.glob('*dark*.fits')
	if ('dark*.fits' not in os.listdir('./')) & (dark == []) : 
		print('No dark today.')
		print('Find exposure time set today...')
		obj = glob.glob('*-000*.fit')
		allexptime = []
		i=0
		...
		print(exptime)
		j=0	
		for j in xrange(len(exptime)) :
			masterdark_loc = '/data3/SAO1m/red/'+camera+'/masterdark/'
			os.system('ls '+masterdark_loc)
			masterdark_list = glob.glob(masterdark_loc+'*dark'+str(int(exptime[j]))+'.fits')
	...
	if (flat == []) :
		print('No flat today')
		print('Find filter set today...')
		obj = glob.glob('*-000*.fit')
		#obj = glob.glob('*flat*.fit')
		allfilter = []
		i=0
		for i in xrange(len(obj)) :
			hdr = fits.getheader(obj[i])
		...
		print(infilter, 'filter set today.')
		j=0
		for j in xrange(len(infilter)) :
			masterflat_loc = '/data3/SAO1m/red/'+camera+'/masterflat_'+infilter[j]+'/'
			os.system('ls '+masterflat_loc)
			masterflat_list = glob.glob(masterflat_loc+'*n'+infilter[j]+'flat.*.fits')
			...
			print(recent_masterflat+' is copied.')
	elif 'skyflat' in flat[0].split('-') :
		skyflat = flat
		os.system('mkdir skyflat')
		skyflatjoin = " ".join(skyflat)
		os.system('mv '+skyflatjoin+' ./skyflat')
	elif 'Domeflat' in flat[0].split('-') :
		domeflat = flat
...
#=================================================================
def SAO_biascom(camera='STX16803'):
	"""
	1. Description 
	...
	iraf.dir('.')
#=================================================================
def SAO_darkcom(camera='STX16803') :
	"""
	1. Description 
	...
	print('Output master '+output_name+' is created.')
#=================================================================
def SAO_flatcom(flattype='sky', process='bias', camera='STX16803') :
	"""
	1. Description 
...
#=================================================================
def SAO_fnamechange(camera='STX16803') :
	"""
	1. Description 
...
#=================================================================
def SAO_filemove(camera='STX16803'):
	"""
	1. Description 
	...
	for n in calframes:
		...
		os.system(chmodcom)
#=================================================================
def SAO_makedir(camera='STX16803'):
	"""
	1. Description
...
def SAO_wcsremap(inim, tmpim):
	...
	return outim
#=================================================================
def SAO_skysex(input_image, mode='single'):
	"""
	1. Description 
...
def geturl(ra, dec, size=240, output_size=None, filters="grizy", format="jpg", color=False):
	...
	return url
#------------------------------------------------------------
def SAO_ps1cut(imlist_name, infilter='r'):
	"""
	1. Description 
	: Sky background subtraction code using SExtractor sky estimation algorithm. When using only one image, mode should be 'single'. When using more than two images, mode should be 'multi'.
...
#=================================================================
def SAO_gregister_inv(imlist_name, infilter='r'):
	"""
	...
	print('Done. \a')
#=================================================================
def SAO_hotpants_public(imlist_name) :
	"""
	1. Description 
